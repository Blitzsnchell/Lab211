...
public class WorkerHandler {
    ...
    private void addWorker(String id, String name, int age, double salary, String workLocation) {        
    ...
    private void changeSalary(String status) {
        ...
        if (status.equals("UP")) {
            ...
            if (w.getSalary() > amount) update = w.getSalary() - amount;
        }
        
        w.setSalary(update);
        shList.add(new SalaryHistory(id, w.getName(), w.getAge(), w.getSalary(), status, java.time.LocalDate.now()));       
    ...
    private List<SalaryHistory> getSortedInformationSalary() {
        // Sort by ID ascendingly, if same ID then sort by salary ascendingly
        ...
        do {
            ...
            switch (choice) {
                case 1:
                    ...
                    addWorker(w.getId(), w.getName(), w.getAge(), w.getSalary(), w.getWorkLocation());
                    break;
                case 2:
                    changeSalary("UP");
                    break;
                ...
                case 4:
                    if (shList.isEmpty()) {
                        inputter.displayErr("Salary History List is empty! Please add informations");
                    } else {
